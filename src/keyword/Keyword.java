 ...
 */
	 ...
	 * 
	 */
	public static void takeScreen_AShot() {
		AShot shot = new AShot();
		Date date = new Date();
		// SimpleDateFormat Format = new SimpleDateFormat("E, dd MMM yyyy, HH_mm_ss");
		// String DateFormat = Format.format(date);
		Constants.screenshot = shot.shootingStrategy(ShootingStrategies.viewportPasting(2000)).takeScreenshot(Constants.driver);
		try {
			ImageIO.write(Constants.screenshot.getImage(), "PNG",new File("E:\\"+date+".png"));
	...
	public static void currentPageAshot() {
		...
		try {
			ImageIO.write(a.getImage(), "JPG", new File("./"+DateAndTime+".jpg"));
		} catch (IOException e) {
    ...
    public static  void ScrollPageVertically(int scrollpx) {
    	JavascriptExecutor js = (JavascriptExecutor) Constants.driver;
    	js.executeScript("window.scrollBy(0,"+scrollpx+")");
    }
    public static  JavascriptExecutor javaScriptExecutescript(String value) {
    	JavascriptExecutor js = (JavascriptExecutor) Constants.driver;
    	return (JavascriptExecutor) js.executeScript(value);
    }
    public static  JavascriptExecutor javaScriptexecuteAsyncScript(String value) {
    	JavascriptExecutor js = (JavascriptExecutor) Constants.driver;
    	return (JavascriptExecutor) js.executeAsyncScript(value);
    }
	/**
	 * 
	 */
